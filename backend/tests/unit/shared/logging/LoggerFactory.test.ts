...
const makeConfig = (overrides: Partial<AppConfig> = {}): AppConfig => ({
  env: 'development',
  runtime: 'local',
  server: { port: 3000, frontendUrl: 'http://localhost:5173' },
  ...
  tink: { clientId: 'id', clientSecret: 'secret', environment: 'sandbox' },
  stripe: { secretKey: 'sk_test', webhookSecret: 'whsec' },
  email: { region: 'us-east-1', fromAddress: 'test@bridge.local' },
  features: { emailEnabled: true, paymentProcessing: true, bankOAuth: true },
  ...overrides,
});
