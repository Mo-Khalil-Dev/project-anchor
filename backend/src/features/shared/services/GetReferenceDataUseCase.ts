...
import type { ReferenceData, NextStep } from '../types/referenceData.types';
...
import { GetAssessmentQuery } from '../../assessment/services/GetAssessmentQuery';
...
export class GetReferenceDataUseCase {
  ...
  async execute(input: { userId: string }): Promise<Result<ReferenceData, Error>> {
    try {
      ...
      this.logger.info('Fetching assessment data', { customerId });
