...
import type {
  IBankDataProvider,
  BankFinancialData,
} from '../../../application/services/IBankDataProvider';
  ...
  async fetchFinancialData(code: string): Promise<Result<BankFinancialData, Error>> {
    ...
    if (tokenResult.isFail) {
      this.logger.error('Failed to exchange OAuth code', {
        error: tokenResult.getError()?.message,
      });
    ...
    if (incomeResult.isFail) {
      this.logger.error('Failed to fetch income report', {
        error: incomeResult.getError()?.message,
      });
    ...
    if (expenseResult.isFail) {
      this.logger.error('Failed to fetch expense check', {
        error: expenseResult.getError()?.message,
      });
    ...
    if (extractedIncome.isFail) {
      this.logger.error('Failed to extract income figures', {
        error: extractedIncome.getError()?.message,
      });
    ...
    if (extractedExpenses.isFail) {
      this.logger.error('Failed to extract expense figures', {
        error: extractedExpenses.getError()?.message,
      });
