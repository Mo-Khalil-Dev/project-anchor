...
export function createAuthRouter(authProvider: IAuthProvider, prisma: PrismaClient): AuthSetup {
  const router = Router();
  ...
  router.get('/auth/initiate-login', asyncHandler(controller.initiateLogin.bind(controller)));

  router.get('/auth/mock-login', asyncHandler(controller.mockLogin.bind(controller)));

  router.post('/auth/callback', asyncHandler(controller.handleCallback.bind(controller)));

  router.get('/auth/callback', asyncHandler(controller.handleCallback.bind(controller)));

  router.get('/auth/refresh', asyncHandler(controller.refreshToken.bind(controller)));

  router.post('/auth/logout', asyncHandler(controller.logout.bind(controller)));
  ...
  router.get(
    ...
    authenticateRequest,
    asyncHandler(controller.getRedirectToJourney.bind(controller))
  );
