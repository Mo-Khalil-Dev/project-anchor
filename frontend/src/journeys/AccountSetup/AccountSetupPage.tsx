...
export function AccountSetupPage() {
  const { step, linkDetails, userName, userInitials, goToLinking, goToHelp, goBackToError, goToRedirect, goToError, goToSuccess, retryLinking, navigateToBankConnection } = useAccountSetupPage();

  if (step === 'checking') return <CheckingState onAdvance={goToLinking} />;
  if (step === 'linking') return <LinkingState userName={userName} userInitials={userInitials} onSuccess={goToSuccess} onError={goToError} />;
  if (step === 'success' && linkDetails) return <SuccessState details={linkDetails} onContinue={goToRedirect} />;
  if (step === 'error' && linkDetails) return <ErrorState details={linkDetails} onRetry={retryLinking} onHelp={goToHelp} />;
